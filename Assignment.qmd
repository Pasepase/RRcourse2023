...
```{r}
load(paste0("C:/Users/serei/Downloads/Data/season_", params$season, ".RData"))
```
...
episodes <- nrow(season_data)

avg_viewers <- mean(season_data$viewers)

#start <- season_data$premiere_date[1, ]
```

Season ``r params$season`` of Game of Thrones consisted of ``r episodes`` episodes that aired between April 17 and June 19, 2011 on HBO. The show gathered an average of 2.515 first-day TV viewers in the US, with the number growing from 2.22 to 3.04 million by the end of the season.
